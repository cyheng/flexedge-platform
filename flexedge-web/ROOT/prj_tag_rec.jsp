...
<div class="layui-tab layui-tab-card" lay-filter="pro_tabs" style="top:0px;">
    <ul class="layui-tab-title">
        ...
        %>
        <li lay-id="22" onclick="show_pro('<%=id%>','<%=tp%>')" title="<%=ttt%>"><%=tt %>
        </li>
        <%
            }
        %>
    ...
    <div class="layui-tab-content">
        <div class="layui-tab-item layui-show">
            <Iframe id="f1" style="width:100%;height:500px;border:0px;overflow: hidden;"
                    src="prj_tag_rec_tss.jsp?prjid=<%=prjid%>&tag=<%=tagpath%>"></Iframe>
        </div>
        <%
            for (RecProL1 rp : rps) {
                String tt = rp.getTitle() + "[" + rp.getName() + "]";
