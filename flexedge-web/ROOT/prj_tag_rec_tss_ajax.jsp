...
<%@ page import="java.util.*,
                 ...
                 cn.doraro.flexedge.core.store.tssdb.*,
                 cn.doraro.flexedge.core.store.record.*,
                 cn.doraro.flexedge.core.alert.*,
                 cn.doraro.flexedge.core.store.gdb.*,
...
<tr id="ai_row_<%=sdt%>" class="file_row" onclick="on_row_clk('<%=sdt%>')" title="">
    <td title="<%=Convert.toXmlValDateStr(fdt) %>"><%=Convert.toHMS(fdt) %>
    </td>
    <td title="<%=Convert.toXmlValDateStr(todt)  %>"><%=Convert.toHMS(todt)  %>
    </td>
    <td><%=(edt - sdt) / 1000  %>
    </td>
    <td><%=(valid ? "√" : "×") %>
