...
<div class="left">
    ...
    <div class="list" onscroll="on_list_scroll()">
        ...
        <div class="tag_item" title="<%=tt%>" tag_id="<%=tagid%>" tag_np="<%=np %>" id="tag_<%=tagid%>"><i
                class="fa fa-square en" style="color:<%=en_c%>" title="<wbt:g><%=en_t%></wbt:g>"></i>
            <span class="tt"><%=np %></span><span class="chk_tag_c"><input type="checkbox" tag_id="<%=tagid %>"
                                                                           class="chk_tag"
                                                                           onclick="on_chk_tag(this,'<%=tagid%>')"/></span>
        </div>
        <%
            }
        %>
...
<div class="mid" onclick="on_pro_clk()">
    <blockquote class="layui-elem-quote "><wbt:g>processor</wbt:g> L1
        <div style="position: absolute;right:10px;top:11px;width:100px;border:0px solid;height:35px;">
            <button type="button" style="top:3px;" class="layui-btn layui-btn-sm " onclick="add_pro_sel(false)">
...
<div class="right">
    ...
    <blockquote class="layui-elem-quote "><wbt:g>processor</wbt:g> L2
        <div style="float: right;margin-right:10px;font: 15px solid;color:#fff5e2">
            <button type="button" style="top:3px;" class="layui-btn layui-btn-sm " onclick="add_pro_sel(true)">
                +<wbt:g>add</wbt:g></button>
        </div>
    </blockquote>
...
<script>
    ...
    function del_pro(prjid, id) {
        //event.stopPropagation();
        let ob = $("#p_" + id);
        dlg.confirm('<wbt:g>del,this,processor</wbt:g> [' + ob.attr("n") + ']?', {
