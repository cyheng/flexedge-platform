...
<div class="left">
    ...
    <div class="station">
        ...
        <div class="prj_item">
            ...
            <button title="read prj from station"
                    onclick="station_up_prj('<%=ps.getId() %>','<%=prjst.getPrjName() %>')">up project
            </button>
            <%
                ...
                if (p_prj != null) {
            %>
            <button title="write prj to station">down project</button>
            <br>
            <%
                }
            %>
            ...
            Interval <input type="number" id="syn_intv_<%=ps.getId() %>_<%=prjst.getPrjName() %>" value="<%=syn_intv %>"
                            style="width:65px"/>
            <br>Failed Keep <input type="checkbox"
                                   id="failed_keep_<%=ps.getId() %>_<%=prjst.getPrjName() %>"  <%=failed_keep_chked %>
                                   style="width:65px"/>
            Max Len <input type="number" id="keep_max_len_<%=ps.getId() %>_<%=prjst.getPrjName() %>"
                           value="<%=keep_max_len %>" style="width:65px"/>
            <button title="set param" onclick="station_prj_set_pm('<%=ps.getId() %>','<%=prjst.getPrjName() %>')">Set
                PM
            </button>
        ...
        <select id="prjlist_<%=ps.getId() %>">
            <%
                for (UAPrj prj : prjs) {
            %>
            <option value="<%=prj.getName()%>">[<%=prj.getName()%>] <%=prj.getTitle()%>
            </option>
            <%
                }
            %>
        ...
        <%
            ...
            for (File tmpf : dataf.listFiles()) {
                if (!tmpf.isDirectory())
                    continue;
        %>
        <option value="data:<%=tmpf.getName() %>">data:<%=tmpf.getName() %>
        </option>
        <%
            }
        %>
...
<script type="text/javascript">
    ...
    function station_prj_start_stop(stationid, prjname, b_start_stop) {
        ...
        send_ajax("pstation_remote_ajax.jsp",
            {
                op: "station_prj_start_stop",
                stationid: stationid,
                prj: prjname,
                start_stop: b_start_stop,
                auto_start: bautostart
            },
            (bsucc, ret) => {
                dlg.msg(ret);
            });
        ...
        send_ajax("pstation_remote_ajax.jsp",
            {
                ...
                prj: prjname,
                auto_start: bautostart,
                data_syn_en: data_syn_en,
                data_syn_intv: data_syn_intv,
                failed_keep: bfailed_keep,
    ...
    function station_syndir_syn(stationid, module, path) {
        ...
        send_ajax("pstation_remote_ajax.jsp",
            ...
            (bsucc, ret) => {
                ...
                cur_syn_mon = {
                    stationid: stationid, module: module, path: path,
                    add_fs: add_fs, update_fs: update_fs, del_fs: del_fs
                };
                dlg.loading(true);
