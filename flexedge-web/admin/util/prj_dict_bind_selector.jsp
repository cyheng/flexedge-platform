...
<form class="layui-form" action="" onsubmit="return false;">
    <blockquote class="layui-elem-quote ">
        <div>Node Path:[<%=path %>] <%=node.getTitle() %>
        </div>
    </blockquote>
    <%
        String dc_names = "";
    ...
    <div class="layui-form-item" id="dc_<%=c_n %>" dc_multi="<%=bmulti%>" dc_input="<%=binput%>">
        <label class="layui-form-label"><%=dc.getClassTitle() %>
        </label>
        <div class="layui-input-inline" style="width:350px;">
            ...
            <%
                    break;
                case i_i:
            %><input type="number" oninput="this.value = this.value.replace(/\-[^0-9]/g, '');" dc_bs="i"
                     id="dc_inp_<%=c_n%>" name="dn_<%=c_n%>" class="layui-input"/><%
                break;
            case i_f:
...
<input dc_name="<%=c_n %>" dn_name="<%=dn.getName() %>" lay-skin="primary" type="<%=tp %>"
       id="dn_<%=dc.getClassName() %>_<%=dn.getName() %>"
       name="dn_<%=c_n%>" title="<%=dn.getTitle() %>" value="<%=dn.getName()%>">
            ...
            <button type="button" class="layui-btn layui-btn-primary layui-btn-sm"
                    onclick="clear_sel('<%=dc.getClassName() %>')" title="<wbt:g>clear</wbt:g>"><i class="fa fa-x"></i>
            </button>
