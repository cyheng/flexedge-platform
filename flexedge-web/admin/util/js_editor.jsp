...
<table class="prop_table">
    <tr>
        <td colspan="2">
            <div id="prop_edit_path" class="prop_edit_path"><%=n.getNodePathTitle() %>
            </div>
        </td>
    </tr>
    <tr>
                    ...
                    <%
                        ...
                        for (PropGroup pg : pgs) {
                            if ("".equals(pg0))
                                pg0 = pg.getName();
                    %>
                    <li><a id="pg_<%=pg.getName() %>" onclick="sel_pg('<%=pg.getName() %>')"><%=pg.getTitle() %>
                    </a></li>
                    <%
                        }
                ...
                <table id="tb_pg_<%=pg.getName() %>" class="pi_edit_table">
                    ...
                    <tr id="pi_<%=pi.getName()%>" onclick="sel_pi('<%=pi.getName()%>')">
                        ...
                        <td style="width:50%">
                            ...
                            %>
                            <input id="piv_<%=pg.getName() %>.<%=pi.getName() %>" class="pi_edit_unit"
                                   type="<%=inptp %>" step="<%=inp_step %>" value="<%=defv %>"
