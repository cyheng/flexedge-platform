...
<blockquote class="layui-elem-quote "><w:g>dict,mgr</w:g>
    <div style="float: right;margin-right:10px;font: 15px solid;color:#fff5e2">
        <button type="button" class="layui-btn layui-btn-sm layui-border-blue" onclick="add_or_edit_dc(null)">
            +<w:g>add,data,class</w:g></button>
...
<div class="left">
    ...
    %>
    <div class="dc_item" id="cid_<%=cid %>" dc_id="<%=cid %>" onclick="on_dc_clk(this)" is_input="<%=bs.isInput()%>"
         dc_title="<%=dc_title%>" dc_name="<%=dc_name %>" bs_title="<%=bs.getTitle() %>">
        <span class="n"></span>
        <span class="t"><%=dc_title %> - <%=dc_name %></span>
        <span class="bs"> [<%=bs.getTitle() %>]</span>
        <span class="op">
	  	<button class="layui-btn layui-btn-xs layui-btn-normal" onclick="add_or_edit_dc('<%=dc.getClassId()%>')"
                title="Edit Data Class"><i class="fa fa-pencil"></i></button>
	      <button class="layui-btn layui-btn-xs layui-btn-normal" onclick="import_dc_txt('<%=dc.getClassId()%>')"
                  title="Import by Txt"><i class="fa-solid fa-file-import"></i></button>
	      <button class="layui-btn layui-btn-xs layui-btn-danger" onclick="del_dc('<%=dc.getClassId()%>')"
                  title="<w:g>del,data,class</w:g>"><i class="fa-regular fa-rectangle-xmark"></i></button>
	  </span>
    </div>
    <%
        }
...
<script>
    ...
    function del_dc(cid) {
        ...
        dlg.confirm('<w:g>del,this,data,class</w:g>?', {
            btn: ["<w:g>yes</w:g>", "<w:g>cancel</w:g>"],
            title: "<w:g>del,confirm</w:g>"
        }, function () {
    ...
    function del_dn(cid, name) {
        event.stopPropagation();
        dlg.confirm('<w:g>del,this,data,item</w:g>?', {
            btn: ["<w:g>yes</w:g>", "<w:g>cancel</w:g>"],
            title: "<w:g>del,confirm</w:g>"
        }, function () {
