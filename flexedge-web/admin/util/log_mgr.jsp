...
<body style="background-color: rgb(238, 243, 249)" topmargin="0" leftmargin="0" rightMargin="0" marginwidth="0"
      marginheight="0">
    ...
    <div class="layui-form-item">
        ...
        <div class="layui-input-inline">
            <select id="def_lvl" lay-filter="def_lvl">
                ...
                %>
                <option value="<%=ILogger.LEVELS[i] %>"><%=ILogger.LEVELS_TT[i]  %>
                </option>
                <%
                    }
                %>
        ...
        <div class="layui-input-mid">
            <button onclick="save_log()" class="layui-btn"
                    title="Save the current configuration and continue to use it at the next startup">Save Config
            </button>
        ...
        <table class="layui-table" lay-size="sm">
            ...
            <tr>
                ...
                <td style="white-space: nowrap">
                    ...
                    <div id="lvl_<%=logid%>">
                        <input onclick="set_lvl('<%=logid%>',<%=ILogger.LOG_LEVEL_TRACE %>)" type="checkbox"
                               id="lvl_trace_<%=logid %>" value="trace"
                               title="Trace"  <%=(u.isTraceEnabled()?"checked=\"checked\"":"") %> lay-ignore>Trace
                        <input onclick="set_lvl('<%=logid%>',<%=ILogger.LOG_LEVEL_DEBUG %>)" type="checkbox"
                               id="lvl_debug_<%=logid %>" value="debug"
                               title="Debug"  <%=(u.isDebugEnabled()?"checked=\"checked\"":"") %> lay-ignore>Debug
                        <input onclick="set_lvl('<%=logid%>',<%=ILogger.LOG_LEVEL_INFO %>)" type="checkbox"
                               id="lvl_info_<%=logid %>" value="info"
                               title="Info"  <%=(u.isInfoEnabled()?"checked=\"checked\"":"") %> lay-ignore>Info
                        <input onclick="set_lvl('<%=logid%>',<%=ILogger.LOG_LEVEL_WARN %>)" type="checkbox"
                               id="lvl_warn_<%=logid %>" value="warn"
                               title="Warn" <%=(u.isWarnEnabled()?"checked=\"checked\"":"") %> lay-ignore>Warn
                        <input onclick="set_lvl('<%=logid%>',<%=ILogger.LOG_LEVEL_ERROR %>)" type="checkbox"
                               id="lvl_error_<%=logid %>" value="error"
                               title="Error"  <%=(u.isErrorEnabled()?"checked=\"checked\"":"") %> lay-ignore>Error
                    </div>
                </td>
            </tr>
            <%
