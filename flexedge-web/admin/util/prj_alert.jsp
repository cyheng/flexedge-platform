...
<div class="left">
    ...
    <div class="list">
        ...
        <div class="tag_item" id="np"><%=np %>
            ...
            %>
            <div class="alert_item" title="<%=tt%>"><i class="fa fa-square en" style="color:<%=en_c%>"
                                                       title="<wbt:g><%=en_t%></wbt:g>"></i>
                <span class="tt"><%=tt %></span><span class="chk_alert_c"><input type="checkbox" id="<%=id %>"
                                                                                 class="chk_alert"
                                                                                 onclick="on_chk_alert('<%=id%>')"/></span>
            </div>
...
<div class="mid" onclick="on_handler_clk()">
    <blockquote class="layui-elem-quote "><wbt:g>alert,handlers</wbt:g>
        <div style="position: absolute;right:10px;top:11px;width:220px;border:0px solid;height:35px;">
            <button id="btn_save" type="button" class="layui-btn layui-btn-sm layui-border-blue layui-btn-primary"
                    onclick="show_alert_his('<%=prjid %>')"><wbt:g>history</wbt:g></button>
            <button id="btn_save" type="button" class="layui-btn layui-btn-sm layui-border-blue layui-btn-primary"
                    onclick="save_h_inout_ids('<%=prjid %>')"><wbt:g>save</wbt:g></button>
            <button type="button" class="layui-btn layui-btn-sm layui-border-blue layui-btn-primary"
                    onclick="add_or_edit_h('<%=prjid %>',null)">+<wbt:g>add</wbt:g></button>
        </div>
    </blockquote>
...
<div class="right">
    <blockquote class="layui-elem-quote "><wbt:g>outputs</wbt:g>
        <div style="float: right;margin-right:10px;font: 15px solid;color:#fff5e2">
            <button type="button" class="layui-btn layui-btn-sm layui-border-blue layui-btn-primary"
                    onclick="add_or_edit_o('<%=prjid %>',null)">+<wbt:g>add</wbt:g></button>
        </div>
    </blockquote>
...
<script>
    ...
    function del_handler(prjid, id) {
        event.stopPropagation();
        dlg.confirm('<wbt:g>del,this,handler</wbt:g>?', {
    ...
    function del_out(prjid, id) {
        event.stopPropagation();
        dlg.confirm('<wbt:g>del,this,output</wbt:g>?', {
            btn: ["Yes", "Cancel"],
            title: "<wbt:g>del,confirm</wbt:g>"
        }, function () {
