...
<table id="view_colorval" class="layui-table" lay-filter="dl_list" lay-size="sm" lay-even="true" style="width:95%"
    ...
    <tr id="lvl_tr_<%=v %>">
        ...
        <td><span id="color_s_<%=v %>"
                  style="width: 30px; height: 30px; background-color: <%=lvl.getColor() %>; cursor: crosshair">&nbsp;&nbsp;&nbsp;&nbsp;</span>
            <input id="color_<%=v %>" value="<%=lvl.getColor() %>" onkeydown="on_color_chg(<%=v %>)"
                   onchange="on_color_chg(<%=v %>)"/>
        </td>
...
<div style="position: relative;right: 30px;float: right;top:50px;">
    <button id="btn_save" class="layui-btn layui-btn-primary" type="button" lay-filter="set_param"
            onclick="save_alert_def()">Save
    </button>
</div>
</body>
