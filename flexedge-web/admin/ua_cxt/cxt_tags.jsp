...
<form class="layui-form" action="" onsubmit="return false;">
    <blockquote class="layui-elem-quote ">&nbsp;
        <div style="left:20px;top:5px;position:absolute;font:bold;font-size: 17"><span
                title=""><%=node.getNodePath() %></span> <span>[<wbt:g>tags_num</wbt:g>:</span><span
                id="tags_num"></span>]
            <a href="javascript:bind_ext('<%=node.getNodePath() %>')" id="node_ext_<%=node.getId() %>"
               title="Set extended properties" style="<%=ext_color%>"><i class="fa fa fa-paperclip"
                                                                         aria-hidden="true"></i></a>
            <a href="javascript:node_access('<%=node.getNodePath() %>')" id="node_access_<%=node.getId() %>"
               title="Access"><i class="fa fa-paper-plane" aria-hidden="true"></i></a>

        </div>
        <div style="left:20px;top:25px;position:absolute;font-size: 15px;"><%=node.getNodePathTitle() %>
        </div>
        <div style="float: right;margin-right:10px;font: 15px solid;color:#fff5e2">
            ...
            <%
                if (b_tags) {
                    ...
                    {
            %>
            <button type="button" class="layui-btn layui-btn-sm layui-border-blue" onclick="add_or_modify_tag('')">
                +<wbt:g>add,tag</wbt:g></button>&nbsp;&nbsp;
            <button type="button" class="layui-btn layui-btn-sm layui-border-blue" onclick="imp_tag()">
                <wbt:g>imp,tag</wbt:g></button>&nbsp;&nbsp;
            <%
                }
            %>
            <button type="button" class="layui-btn layui-btn-sm layui-border-blue" onclick="add_or_modify_tag('',true)">
                +<wbt:g>add,middle,tag</wbt:g></button>&nbsp;&nbsp;
            <%--
            <button type="button" class="layui-btn layui-btn-sm layui-btn-danger" onclick="del_tag()">+Delete Tag</button>
             --%>
            <input type="checkbox" id="show_sys" name="show_sys" lay-filter="show_sys" lay-skin="switch"
                   lay-text="<wbt:g>hide,system,tags</wbt:g>|<wbt:g>show,system,tags</wbt:g>"/>
            <input type="checkbox" id="show_sub" name="show_sub" lay-filter="show_sub" lay-skin="switch"
                   lay-text="<wbt:g>hide,sub,tags</wbt:g>|<wbt:g>show,sub,tags</wbt:g>"/>

            &nbsp;&nbsp;<button type="button" class="layui-btn layui-btn-sm layui-border-blue" onclick="to_excel()"><i
                class="fa fa-arrow-right"></i>Excel
        </button>
...
<script>
                ...
                $(this).selectMenu({
                    ...
                    data: () => {
                        ...
                        if (get_selected_tagids() != '')
                            d.push({
                                content: '<wbt:g>copy,selected,tag</wbt:g>', callback: () => {
            ...
            if (3 == e.which) {
                ...
                $(this).selectMenu({
                    ...
                    data: () => {
                        ...
                        {
                            return [{
                                content: '<wbt:g>rename,tag</wbt:g>', callback: () => {
                        ...
                        if (t_loc && !t_sys) {
                            var r = [];
                            r.push({
                                content: '<wbt:g>modify,tag</wbt:g>', callback: () => {
                                    add_or_modify_tag(t_id);
                                }
                            });

                            r.push({
                                content: '<wbt:g>copy,tag</wbt:g>', callback: () => {
                            ...
                            if (get_selected_tagids() != '') {
                                ...
                                r.push({
                                    content: '<wbt:g>move,selected,tag</wbt:g>', callback: () => {
                                        move_tag(get_selected_tagids());
                                    }
                                });

                                r.push({
                                    content: '<wbt:g>del,selected,tag</wbt:g>', callback: () => {
                            ...
                            r.push({
                                content: '<wbt:g>del,tag</wbt:g>', callback: () => {
                                    del_tag(t_id);
                                }
                            });
    ...
    function move_tag(tagids) {
        dlg.open("../util/prj_tree_node_sel.jsp?prjid=" + prjid, {
                title: "<wbt:g>select,move,tar</wbt:g>",
                w: '500px',
                h: '400px'
            },
    ...
    function del_tag(id) {
        ...
        ret.id = id;
        dlg.confirm("<wbt:g>del,selected,tags</wbt:g>？", {
    ...
    function refresh_tags() {
        if (!b_tags)
            return;
        dlg.loading(true);
        send_ajax("cxt_tags_tb_ajax.jsp", {
            path: cxt_path,
            sys: b_sys,
            sub: b_sub,
            sortby: sort_by
    ...
    function rec_tag_set(ob, tag, tt) {
        ...
        dlg.open("../store/rec_tag_param_edit.jsp?prjid=" + prjid + "&tag=" + tag,
            ...
            [
                ...
                function (dlgw) {
                    dlg.confirm('<wbt:g>unset,this,recorder</wbt:g>?', {
                        btn: ["<wbt:g>yes</wbt:g>", "<wbt:g>cancel</wbt:g>"],
                        title: "<wbt:g>unset,confirm</wbt:g>"
                    }, function () {
                        send_ajax("../store/rec_tag_param_ajax.jsp", {
                            op: "unset",
                            prjid: prjid,
                            tag: tag
                        }, function (bsucc, ret) {
