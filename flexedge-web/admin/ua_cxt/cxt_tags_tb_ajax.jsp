...
>
    ...
    <td style="white-space: nowrap;">
        <%
            if (tag.isCanWrite()) {
        %>
        <input type="text" id="ctag_w_<%=tag.getId()%>" value="" size="8" style="color:#999999"/><a
            href="javascript:w_tag('<%=tag.getId()%>')"><i class="fa fa-pencil-square" aria-hidden="true"></i></a>
        <%
            }
        %>
    </td>
    <td style="white-space: nowrap;">
        ...
        %>&nbsp;<a href="javascript:bind_ext('<%=tagpath%>')" id="node_ext_<%=tag.getId() %>" title="<%=tagt %>"
                   style="<%=ext_color%>"><i class="fa-solid fa-paperclip" aria-hidden="true"></i></a>
        &nbsp;<a href="javascript:node_access('<%=tagpath%>')" title="<wbt:g>access</wbt:g>"><i
    ...
    %>
        <button onclick="rec_tag_set(this,'<%=tmppath %>','<%=tag.getTitle() %> [<%=tmppath %>]')"
                title="<wbt:g>set,tag,recorder</wbt:g>" style="color:<%=color%>">&nbsp;<i class="fa fa-edit"/></i>&nbsp;
        </button>
        <span id="rec_tag_show_<%=tmppath %>" style="display:<%=dis_show%>;">
<a href="javascript:rec_tag_show('<%=tmppath %>','<%=tag.getTitle() %> [<%=tmppath %>]')"
   title="<wbt:g>show,recorded,history</wbt:g>">&nbsp;<i class="fa fa-line-chart"></i></a>
    ...
    <td>
        ...
        %><a href="javascript:show_data_his('<%=outtp %>','<%=outid %>','<%=tagp%>','<%=tt %>')"
             title="<wbt:g>show,history</wbt:g> - <%=outtpt%>">&nbsp;<i class="fa fa-line-chart"/></i>&nbsp;</a>
        <%
                }
            }
        %>
