...
<select id="search_unit">
    ...
    <%
        for (ValUnit vu : ValUnit.values()) {
            String vun = vu.name();
    %>
    <option value="<%=vun%>"><%=vu.getUnit()%> <%=vu.getTitle() %>
    </option>
    <%
        }
    %>
    ...
    <table width='100%' border='1' height0="100%">
        ...
        <tr class="row_ntags">
            <td colspan="8">&nbsp;<%=subpath%>
                <button id="btn_<%=subpath %>" subpath="<%=subpath %>"
                        onclick="sel_all_or_not(this,'<%=subpath %>',false)">All children
                </button>
                <button id="btn_<%=subpath %>" subpath="<%=subpath %>"
                        onclick="sel_all_or_not(this,'<%=subpath %>',true)">All offspring
                </button>
            </td>
        </tr>
        <%
        ...
        %>
        <tr id="row_<%=pathn %>" tagid="<%=tagid %>" onmouseover="mouseover(this)" onmouseout="mouseout(this)"
            onclick="clk_sel(this)">
            <td><input type="checkbox" class="chk" id="chk_<%=tagid %>" tagid="<%=tagid %>" path="<%=pathn %>"
                       sub="<%=subpath %>" <%=chked %>/></td>
            <td><%=pathn %>
            </td>
            <td><%=addr %>
            </td>
