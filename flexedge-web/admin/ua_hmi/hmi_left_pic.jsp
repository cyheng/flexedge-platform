...
<table width='100%' height='100%'>
    ...
    <tr>
        <td valign="top" width="25%">分类
            <select id='var_cat' multiple="multiple" style="width: 100%;height: 100%"
                    onchange="single_sel_chg_cat('var_cat')">
                <%
                    for (GRCat grc : GRManager.getInstance().getGRCatAll()) {
                        if (grc.getName().equals(cat)) {
                %>
                <option value="<%=grc.getName() %>" selected="selected"><%=grc.getTitleCN() %>
                </option>
                <%
                } else {
                %>
                <option value="<%=grc.getName() %>"><%=grc.getTitleCN() %>
                </option>
                <%
                        }
                    }
        ...
        <td valign="top" width="25%">图元
            <select id='var_item' multiple="multiple" style="width: 100%;height: 100%"
                    onchange="single_sel_chg('var_item')">
                <%
                    if (curCat != null) {
                        for (GRItem gri : curCat.getGRItems()) {
                %>
                <option value="<%=gri.getRefPath() %>"><%=gri.getTitleCN() %>
                </option>
                <%
                        }
                    }
