...
<blockquote class="layui-elem-quote " id="selected_info">&nbsp;
    <wbt:g>selected,cat</wbt:g>:<span id="selected_libcat_tt"
                                      style="color:red"><%=sel_lib_tt %> - <%=sel_cat_tt %></span>
    <%
        if (bsel_comp) {
    %>
    <wbt:g>selected</wbt:g> HMI <wbt:g>comp</wbt:g>:<span id="selected_comp_tt"
                                                          style="color:red"><%=sel_comp_tt %></span>
    <%
        }
    %>
</blockquote>
...
<table style="width:99%;height:<%=tbh %>;border:0px;">
    <tr>
        <td style="width:40%;height:100%">
            <iframe name="comp_left" src="comp_lib_left.jsp?edit=<%=bedit %>&libid=<%=libid %>&catid=<%=sel_catid %>"
        ...
        <td style="width:60%;height:100%;vertical-align: top;" class="oc-toolbar">
            ...
            <div id="right_add_div" style="float:left;margin-left:8px;margin-top:4px;width:100%;display:none;">
                ...
                <a id="top_oper_add_comp" class="layui-btn  layui-btn-sm layui-btn-primary "><i
                        class="fa-regular fa-square-plus"></i>&nbsp;<wbt:g>add_comp</wbt:g></a>
            </div>
            <%
                }
            %>
...
<table style="width:99%;height:<%=tbh %>;border:0px;">
    <tr>
        <td style="width:100%;height:100%">
            <iframe name="comp_left" src="comp_lib_left.jsp?edit=<%=bedit %>&libid=<%=libid %>"
                    style="width:100%;height:100%;border:0"></iframe>
        </td>
    </tr>
</table>
...
<script type="text/javascript">
    ...
    function del_comp(id) {
        dlg.confirm('<wbt:g>del,this,item</wbt:g>?', {
            ...
            if (3 == e.which) {
                ...
                $(this).selectMenu({
                    ...
                    data: () => {
                        ...
                        var r = [];
                        r.push({
                            content: '<wbt:g>modify</wbt:g>', callback: () => {
                                edit_comp(compid);
                            }
                        });

                        r.push({
                            content: '<wbt:g>copy</wbt:g>', callback: () => {
                        ...
                        r.push({
                            content: '<wbt:g>del</wbt:g>', callback: () => {
                                del_comp(compid);
                            }
                        });
