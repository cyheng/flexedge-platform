...
<%@ page import="java.util.*,
                 ...
                 cn.doraro.flexedge.driver.common.modbus.sim.*" %>
<%!
...
<blockquote class="layui-elem-quote ">Simulation Instance [<%=ins.getTitle() %>]
    ...
    <%
        ...
        if (ins.isEnable()) {
    %>
    <span id="" style="width:20px;height:20px;color: <%=run_c0%>;"><i
            class="fa fa-cog <%=run_icon0 %> fa-lg"></i></span>&nbsp;<%=run_t0%>

    <%
        if (ins.RT_isRunning()) {
    %>

    <i id="prj_btn_stop" class="fa fa-pause fa-lg" style="color:red" title="stop"
       onclick="ins_start_stop(false,'<%=ins.getId()%>')"></i>

    <%
    } else {
    %>
    <i id="prj_btn_start" class="fa fa-play fa-lg" style="color:green" title="start"
       onclick="ins_start_stop(true,'<%=ins.getId()%>')"></i>
    <%
        }
    } else {
            ...
            <td style="width:50%;height:100%;" valign="top">
                <div style="width:100%;height:100%;overflow: auto;">
                    ...
                    <div class="btn-group open " id="menu_tree_add_ch" style="border:solid 1px;">
                        <a class="btn layui-btn layui-btn-sm" href="#"><i class="fa fa-exchange fa-lg"></i> +Add
                            Channel(Bus)</a>
                        <a class="btn layui-btn layui-btn-sm" href="#">
                            <span class="fa fa-caret-down" title="Toggle dropdown menu"></span>
                        </a>
                    </div>
                    <button onclick="edit_ins_js('init')"
                            class="layui-btn layui-btn-<%=(false?"normal":"primary")%> layui-border-blue layui-btn-sm">
                        init script
                    </button>
                    <button onclick="edit_ins_js('run')"
                            class="layui-btn layui-btn-<%=(false?"normal":"primary")%> layui-border-blue layui-btn-sm">
                        run in loop script
                    </button>
                    <button onclick="edit_ins_js('end')"
                            class="layui-btn layui-btn-<%=(false?"normal":"primary")%> layui-border-blue layui-btn-sm">
                        end script
                    </button>
                    ...
                    <table class="layui-table">
                        ...
                        <tr>
                            <th><i class="fa fa-exchange fa-lg"></i><%=ch.getTitle()%> - <%=ch.getName()%>
                                <a href="javascript:add_or_edit_ch('<%=ch_tp %>','<%=ch_tpt %>','<%=ch.getId()%>')"><i
                                        title="edit channel" class="fa fa-pencil-square fa-lg " aria-hidden="true"></i></a>
                            </th>
                            <th>
                                <%=connt %>
                                <a onclick="ch_conn_set('<%=ins.getId()%>','<%=ch.getId() %>')"><i title="edit channel"
                            ...
                            <th>
                                <a href="javascript:ch_del('<%=ch.getId()%>')" style="color:red"><i
                                        title="delete channel" class="fa fa-times fa-lg " aria-hidden="true"></i></a>
                            </th>
                            <th>
                                <a href="javascript:add_or_edit_dev('<%=ch_tp %>','<%=ch_tpt %>','<%=ch.getId()%>')"><i
                                        title="add device" class="fa-solid fa-square-plus fa-lg" aria-hidden="true"></i></a>
                            </th>
                        </tr>
                        </thead>
                        <%
                        ...
                        <tr title="<%=ta.getTitle()%>">
                            <td><img src="../inc/sm_icon_dev.png"/><%=ta.getName() %><a
                                    href="javascript:add_or_edit_dev('<%=ch_tp %>','<%=ch_tpt %>','<%=ch.getId() %>','<%=ta.getId() %>')"><i
                                    title="edit device" class="fa fa-pencil-square fa-lg " aria-hidden="true"></i></a>
                            </td>

                            <td colspan="2">
                                <%=ta.getDevTitle() %>
                                <a onclick="dev_setup('<%=ch_tp %>','<%=ch_tpt %>','<%=ch.getId() %>','<%=ta.getId() %>')"><i
                                        title="setup device" class="fa fa-pencil-square fa-lg " aria-hidden="true"></i></a>
                                <button onclick="dev_tags_edit('<%=ch_tp %>','<%=ch_tpt %>','<%=ch.getId() %>','<%=ta.getId() %>')"
                                        class="layui-btn layui-btn-<%=(false?"normal":"primary") %> layui-border-blue layui-btn-sm">
                                    Tags
                                </button>
                            ...
                            <td>
                                <a href="javascript:dev_del(''<%=ch.getId() %>','<%=ta.getId() %>')"
                                   title="delete device" style="color:red"><i class="fa fa-times fa-lg "
                                                                              aria-hidden="true"></i></a>
                            </td>
                            <td>


                                <a id="dev_rt_<%=ta.getId() %>"
                                   href="javascript:dev_rt('<%=ch_tp %>','<%=ch.getId() %>','<%=ta.getId() %>')"
                                   title="device runtime" style="color:#00537c"><i id="dev_run_icon"
...
<script>
    ...
    var tree_menu = [
        ...
        {
            content: 'Modbus Slave', callback: function () {
                add_or_edit_ch('mslave', 'ModbusSlave', '');
            }
        },
        {content: 'sm_divider'},
