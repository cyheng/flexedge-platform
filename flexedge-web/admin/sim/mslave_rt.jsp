...
<div class="layui-collapse">
    <div class="layui-colla-item">
        <h2 class="layui-colla-title"><%=fc%>:<%=fct %> Address:<%=regidx %> Length:<%=regnum %>
        </h2>
        <div class="layui-colla-content layui-show">
            <%
                for (int i = 0; i < regnum; i++) {
                    ...
                    if (st != null)
                        tagn = st.getName();
            %><%=addr_str %>: <span id="r_<%=segid%>_<%=i%>" onclick="set_reg_val('<%=segid%>',<%=i%>)"
                                    style="cursor:hand;"></span>&nbsp;
            <span id="tag_<%=segid%>_<%=i%>" onclick="set_reg_tag('<%=segid%>',<%=i%>)"
                  style="cursor:hand;">T&nbsp;<%=tagn %></span>
            <%
                }
            %>
        </div>
