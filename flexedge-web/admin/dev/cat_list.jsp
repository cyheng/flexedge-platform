    ...
    <table id="top_oper">
        <tr>
            <td style="padding-left: 5px;">
                <button id="top_oper_add_dev" class="layui-btn  layui-btn-xs layui-btn-primary "><i
                        class="fa-regular fa-square-plus"></i>&nbsp;<wbt:g>add,dev</wbt:g></button>
            </td>
            <td>
...
%>
<table id="view_colorval" class="layui-table" lay-filter="dl_list" lay-size="sm" lay-even="true" style="width:100%"
       border="1">
    ...
    <tr id="devdef_" onclick="on_clk_def('<%=ddid%>','<%=dd.getName() %>','<%=dd.getTitle() %>')">
        ...
        <td>
            <%
                if (bedit) {
            %>
            <button type="button" class="layui-btn layui-btn-xs layui-btn-normal" lay-event="edit basic"
                    onclick="edit_devdef('<%=ddid%>')" title="Edit Basic"><i class="fa fa-pencil"></i></button>

            <button type="button" class="layui-btn layui-btn-xs layui-btn-normal" lay-event="edit detail"
                    onclick="edit_devdef_detail('<%=ddid%>')" title="Edit Detail"><i
                    class="fa-regular fa-pen-to-square"></i></button>

            <button type="button" class="layui-btn layui-btn-xs layui-btn-danger" lay-event="del" title="delete"
                    onclick="del_devdef('<%=ddid%>')"><i class="fa-regular fa-rectangle-xmark"></i></button>
            <%
            } else {
            %>
            <button type="button" class="layui-btn layui-btn-xs" lay-event="del" title="select device"><i
...
<script type="text/javascript">
    ...
    function del_devdef(id) {
        dlg.confirm('<wbt:g>del,this,dev</wbt:g>?', {
