        ...
        <tr>
            ...
            <td style0="width:115px;">
                <form class="layui-form" action="" onsubmit="return false;">
                    <select id="lib_ids" lay-filter="lib_ids" lay-ignore0 style="width:100px;">
                        ...
                        %>
                        <option value="<%=lib0.getId()%>" <%=seled %>><%=lib0.getTitle() %>
                        </option>
                        <%
                            }
                        %>
            ...
            <td style="padding-left: 15px;">
                <%
                    if (bedit) {
                %>
                <button id="top_oper_add_cat" type="button" class="layui-btn layui-btn-xs layui-btn-primary"
                        lay-event="add_cat"><i class="fa fa-plus"></i><wbt:g>cat</wbt:g></button>
                <%
                    }
                %>
            </td>
...
<script type="text/html" id="row_toolbar">
    <div class="layui-btn-group">
        ...
        <button type="button" class="layui-btn layui-btn-xs layui-btn-normal" lay-event="edit"><i
                class="fa fa-pencil"></i></button>

        <button type="button" class="layui-btn layui-btn-xs layui-btn-danger" lay-event="del" title="delete"><i
...
<script>
    ...
    $('#btn_menu_lib').click(function () {
        $(this).selectMenu({
            ...
            data: [
                ...
                {
                    content: '<wbt:g>edit,current,lib</wbt:g>', callback: function () {
                        lib_edit();
                    }
                },
                {
                    content: '<wbt:g>add,lib</wbt:g>', callback: function () {
                        lib_add();
                    }
                },
                {content: '<wbt:g>cat</wbt:g>', header: true},
                {
                    content: '<wbt:g>add,cat</wbt:g>', callback: function () {
                        lib_add_edit_cat();
                    }
                }
            ]
    ...
    function lib_del_cat(cid) {
        //var libid = $("#lib_ids").val() ;
        dlg.confirm('<wbt:g>del,this,cat</wbt:g>?', {
