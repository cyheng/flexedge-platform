...
//        	throw new
    ...
    private static MCmdAsyn readFrom(String filebase, InputStream s, boolean readfile) throws Exception {
        ...
        if (fnstr != null && !fnstr.equals("")) {//has files
            ...
            while (st.hasMoreTokens()) {
                ...
                if (readfile) {//remote tranfer
                    File tmpf = new File(filebf, fn);
                    readStreamToFile(tmpf, flen, s);
                    //
                    fls.add(tmpf);
                } else {//pack or unpack local,file location is not change
                    if (sorpath == null)
                        continue;
    ...
    public String getHeadStr() {//[k1=a1&b=xx]
