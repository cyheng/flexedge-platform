package cn.doraro.flexedge.core.util.queue;
        ...
        public void run() {
            ...
            while (procTh != null) {
                ...
                try {
                    ...
                    if (hr == HandleResult.Handler_Invalid) {//处理器本身无效,
...
//					处理成功
    ...
    private QueItem<T> peekQueue() {
        if (normalList.isEmpty()) {
            ...
            if (retryList.isEmpty()) {
                ...
                if (r_qi.startProcT > System.currentTimeMillis()) {//重新尝试列表的内容需要等待,normal优先级高
                    lastPeekNormal = true;
                    return n_qi;
                }
    ...
    private synchronized QueItem<T> dequeue_qi() {//为了避免retry列表被忽略,dequeue支持交替的算法.
