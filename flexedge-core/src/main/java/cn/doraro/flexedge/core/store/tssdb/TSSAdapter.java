...
//				ss.add(spk.segs);
    ...
    public <T> void addTagValue(String tag, long dt, boolean bvalid, T val) {
        ...
        if (bvalid) {// 无效状态因为值可以认为是不变的，也就不需要gap控制数据量了
    ...
    public static interface ISavedListener {
        /**
         * @param b_insert    true=insert false=update
         * @param tagsegs
         * @param valseg
         * @param saved_enddt 结束时间不能使用valseg，很可能并发情况下，valseg中的值会被改变
         */
