...
import cn.doraro.flexedge.core.conn.html.HtmlBlockLocator.TracePoint;
    ...
    private void clearNode(Node curn) {
        ...
        for (Node node : nodes) {
            ...
            if (node instanceof Element) {
                ...
                if ("script".equals(nn)) {//clear all script
                    delnodes.add(node);
                    continue;
                }
            }
    ...
    private void findNodesByTracePts(Node curn, HashMap<TracePoint, List<Node>> tp2ns, List<TracePoint> tps) {
        ...
        if (curn instanceof TextNode) {
            ...
            for (TracePoint tp : tps) {
                if (wtxt.contains(tp.getTraceTxt())) {//System.out.println(" find txt="+wtxt+" contains="+tp.getTraceTxt()) ;
