    ...
    public String getSqlType(XmlValType vt, int maxlen) {
        if (vt == XmlValType.vt_int32) {
            if (maxlen > 0)
                return "int(" + maxlen + ")";
            return "integer";
        } else if (vt == XmlValType.vt_byte_array) {//blob
            return "longblob";
