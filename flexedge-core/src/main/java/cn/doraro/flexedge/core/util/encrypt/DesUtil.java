package cn.doraro.flexedge.core.util.encrypt;
...
*/
    ...
    void F(byte[] FR, byte[] FK, byte[] Output) {
        ...
        for (n = 0; n < 8; n++) {
            ...
            for (h = 0; h < 4; h++) {//System.out.println ("j="+j+" h="+h) ;
                switch (h) {
                    case 0:
                        Temp2[j + h] = (byte) ((SBoxes[n][Row][Column] & 8) / 8);
                        break;
                    case 1:
                        Temp2[j + h] = (byte) ((SBoxes[n][Row][Column] & 4) / 4);
                        break;
                    case 2:
                        Temp2[j + h] = (byte) ((SBoxes[n][Row][Column] & 2) / 2);
                        break;
                    case 3:
                        Temp2[j + h] = (byte) ((SBoxes[n][Row][Column] & 1));
                        break;
                }
            }
        }
